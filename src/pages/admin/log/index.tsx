...
import MenuSelect from '@/pages/admin/menu/components/MenuSelect'
import UserSelect from '@/pages/admin/user/components/UserSelect'
...
const TableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      ...
      dataIndex: 'user',
      render: (_, record) => (
          <Tag color="success" key={record.user.id}>{record.user.name}</Tag>
      ),
    ...
    },
    {
      ...
      dataIndex: 'menu',
      render: (_, record) => (
          <Tag color="processing" key={record.menu.id}>{record.menu.name}</Tag>
      ),
      renderFormItem: (item, { defaultRender, ...rest }, form) => {
    ...
    },
    {
      ...
      dataIndex: 'method',
      render: (_, record) => (
          <Tag color="warning" key={record.method.id}>{record.method.name}</Tag>
      ),
      hideInSearch: true
    },
    {
      ...
      renderText: (val: string) => {return `${val.substring(0, 19)}` }
    },
