...
import RoleSelect from '../role/components/RoleSelect';
...
const TableList: React.FC<{}> = () => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      ...
      valueType: 'dateRange',
      renderText: (val: string) => {return `${val.substring(0, 10)}` }
    },
    {
      title: '操作',
      dataIndex: 'option',
      ...
      render: (_, record) => (
        <>
          ...
          <a onClick={() => {
            handleOption(record, '重置密码', resetPassword) 
            }}>
            重置密码
          </a>
          <Divider type="vertical" />
    ...
    <Dropdown
      overlay={
        <Menu onClick={({ key }) => handleMore(key, item)}>
          {item.id != 1 && item.status === false && (<Menu.Item key="status">启用</Menu.Item>)}
          {item.id != 1 && item.status === true && (<Menu.Item key="status">禁用</Menu.Item>)}
